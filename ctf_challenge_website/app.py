...
flags = {
    ...
    'challenge2': 'flag{XSS_Challenge_Completed}',
...
def submit_flag():
    ...
    if challenge not in flags:
        return jsonify({'correct': False, 'message': 'Invalid challenge'})

    if flag == flags[challenge]:
        if challenge not in user_solved_challenges:
            user_solved_challenges[challenge] = True
            total_points += 1
            return jsonify({'correct': True, 'total_points': total_points})
        else:
            return jsonify({'correct': False, 'message': 'Challenge already solved'})
    else:
